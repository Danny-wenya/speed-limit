...
class DQNMain(MainFuncBase):
    ...
    def real_sampling(self,epoch):
        ...
        for ite in range(self.SIMULATE_TIME):
            ...
            # 按照时间顺序保存文件
            json.write([next_s, reward, ist, infos],f)
    ...
    def evaluate(self, model, epoch):
        while True:
            try:
                return self.real_evaluate(model, epoch)
            except Exception as e:
                # print(e)
                # raise e
                if self.DEV:
    ...
    def real_evaluate(self, model_name,epoch):
        with torch.no_grad():
            state, infos = self.test_env.reset()
            action = self.agent.action(state, model_name = model_name, evaluate = True)
            tot_reward = np.zeros(self.THREADS, dtype=float)
            with open(f'./gcndata_epoch{epoch}.json','w') as f:
                for ite in range(self.SIMULATE_TIME):
                    next_s, reward, ist, infos = self.test_env.step(action)
                    data=[]
                    for state in next_s[0]:
                        data.append([state[key] for key in ['TSflow','TSwait','TSgreen','TSphase','TStime','Envtime']])
                    try:
                    ...
                    next_action = self.agent.action(next_s, model_name = model_name, evaluate = True)
                    action = next_action
                    state = next_s
